...
export async function GET(req: Request) {
  try {
    ...
    if (!mongoose.models.Teacher) mongoose.model('Teacher', Teacher.schema);
    if (!mongoose.models.Subject) mongoose.model('Subject', Subject.schema);
    if (!mongoose.models.Student) mongoose.model('Student', Student.schema);
    ...
    if (!academicYearId || academicYearId === 'undefined') {
      const currentYear = await Annee.findOne({ status: 'En cours' });
      academicYearId = currentYear?._id.toString(); // Conversion en string pour éviter les problèmes
    }
    ...
    const childrenWithAttendance = await Promise.all(
      parent.children.map(async (childId: any) => {
        ...
        if (academicYearId) attendanceQuery.academicYear = academicYearId;
        if (period && period !== 'Toutes') attendanceQuery.period = period;

        const records = await Attendance.find(attendanceQuery)
        .populate({ path: 'subjectId', model: 'Subject', select: 'name' }) // Précisez 'model' ici
        .populate({ path: 'teacherId', model: 'Teacher', select: 'name' }) // Précisez 'model' ici
        .sort({ date: -1 })
        .lean();
